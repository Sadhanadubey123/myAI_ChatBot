...
# -----------------------------
def init_db():
    """Initialize the SQLite DB if not already created."""
...
def fetch_from_web(query: str) -> str:
    """
    Fetch an answer using DuckDuckGo Search (free, no API key required).
    """
    try:
        with DDGS() as ddgs:
            results = list(ddgs.text(query, max_results=3))
            if results:
                return results[0]["body"]  # snippet text
        return "No snippet found in DuckDuckGo results."
